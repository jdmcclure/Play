...
SELECT
	...
	,UPPER(ArrestPerson.lastName + ',' + ArrestPerson.firstName + ' ' + CASE WHEN ArrestPerson.middleName IS NULL THEN '' 
			ELSE ArrestPerson.middleName END) AS 'Name'
...
WHERE
	...
	AND
	ArrestOfficer.involvementDate BETWEEN DATEADD(DAY, DATEDIFF(day, 1, '{date_input}'), '07:00:00')
	AND DATEADD(DAY, DATEDIFF(day, 0, '{date_input}'), '07:00:00')
...
db_connection_string = f"mssql+pyodbc://{username}:{password}@{server}/{database}?driver=ODBC+Driver+17+for+SQL+Server&protocol=tcp"
engine = create_engine(db_connection_string)

df = pd.read_sql_query(sql=text(query), con=engine.connect())

file_format = date_input.replace("-", "_")

excel_file = f"\\\wsdev.colostate.edu\\cwis165\\WWWROOT\\App_Data\\{file_format}.xlsx"
